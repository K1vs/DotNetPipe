...
internal class VirtualClass
{
    ...
    protected virtual async Task ProcessDigits(string input)
    {
    ...
    protected virtual async Task ProcessLetters(string input)
    {
    ...
    protected virtual async Task ProcessSpecialChars(string input)
    {
    ...
    protected virtual async Task ProcessDefault(char[] input)
    {
...
internal class VirtualClassWithMutation : VirtualClass
{
    ...
    protected override async Task ClassifyStringContent(string input)
    {
        var containsOnlyDigits = !string.IsNullOrEmpty(input) && input.All(char.IsDigit);
    ...
    protected override async Task ProcessDigits(string input)
    {
        var number = int.TryParse(input, out var parsed) ? parsed : 0;
    ...
    protected override async Task ProcessLetters(string input)
    {
    ...
    protected override async Task ProcessSpecialChars(string input)
    {
        var noWhitespace = new string(input.Where(c => !char.IsWhiteSpace(c)).ToArray());
    ...
    protected override async Task ProcessDefault(char[] input)
    {
        var digitCount = input.Count(char.IsDigit);
        var letterCount = input.Count(char.IsLetter);
        var ratio = letterCount > 0 ? digitCount / letterCount : digitCount;
