...
namespace DotNetPipe.Benchmarks.Async.TestPipelines.WithMutatorsIfElseFast;
...
internal class VirtualClass
{
    ...
    protected virtual async Task<int> CheckIntOrFloat(string input)
    {
    ...
    protected virtual async Task<int> FloatProcessing(string input)
    {
        // ParseFloat step
    ...
    protected virtual async Task<int> ParseIntOrDefault(int input)
    {
...
{
    protected override async Task<int> CheckIntOrFloat(string input)
    {
    ...
    protected override async Task<int> FloatProcessing(string input)
    {
        // ParseFloat step
        if (double.TryParse(input, out var floatValue))
        {
    ...
    protected override async Task<int> ParseIntOrDefault(int input)
    {
        // Mutation: multiply by (multiplier + 2) instead of just multiplier
        return input * (_multiplier + 2);
    }

    protected override async Task TestHandler(int input)
    {
