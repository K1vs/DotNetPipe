...
namespace DotNetPipe.Benchmarks.Universal.TestPipelines.WithMutatorsForkFast;
...
{
    protected override async ValueTask<int> ParseToInt(string input)
    {
        if (int.TryParse(input, out var number))
        {
    ...
    protected override async ValueTask<string> AddSpaces(string input)
    {
