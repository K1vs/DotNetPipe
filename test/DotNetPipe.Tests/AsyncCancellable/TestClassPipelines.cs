...
{
    private readonly Func<int, CancellationToken, Task> _handler;

    public TestHandlerStep(Func<int, CancellationToken, Task> handler)
    {
        _handler = handler;
    }
    ...
    public async Task Handle(int input, CancellationToken ct = default)
    {
...
public class TestPipeline : IPipeline<int>
{
    ...
    public TestPipeline(IEnumerable<IMutator<Space>> mutators, Func<int, CancellationToken, Task> handler)
    {
        _handler = handler;
    ...
    public async Task Run(int input, CancellationToken ct = default) => await _compiledPipeline(input, ct);
}
...
public class TestLinearStep : ILinearStep<int, int>
{
    ...
    public async Task Handle(int input, Handler<int> next, CancellationToken ct = default)
    {
...
public class TestThreeStepPipeline : IPipeline<int>
{
    ...
    public TestThreeStepPipeline(IEnumerable<IMutator<Space>> mutators, Func<int, CancellationToken, Task> handler)
    {
        _handler = handler;
        var pipeline = CreateAndCompilePipeline();
        _compiledPipeline = pipeline.Compile(cfg =>
...
public class TestTwoStepPipeline : IPipeline<int>
{
    ...
    public TestTwoStepPipeline(IEnumerable<IMutator<Space>> mutators, Func<int, CancellationToken, Task> handler)
    {
        _handler = handler;
        var pipeline = CreateAndCompilePipeline();
        _compiledPipeline = pipeline.Compile(cfg =>
...
public class TestStringParseLinearStep : ILinearStep<string, int>
{
    ...
    public async Task Handle(string input, Handler<int> next, CancellationToken ct = default)
    {
...
public class TestStringParsingPipeline : IPipeline<string>
{
    ...
    public TestStringParsingPipeline(IEnumerable<IMutator<Space>> mutators, Func<int, CancellationToken, Task> handler)
    {
        _handler = handler;
        var pipeline = CreateAndCompilePipeline();
        _compiledPipeline = pipeline.Compile(cfg =>
    ...
    public async Task Run(string input, CancellationToken ct = default) => await _compiledPipeline(input, ct);
}
...
public class TestTrimStringLinearStep : ILinearStep<string, string>
{
    ...
    public async Task Handle(string input, Handler<string> next, CancellationToken ct = default)
    {
...
public class TestCheckIntOrFloatIfStep : IIfStep<string, string, int>
{
    ...
    public async Task Handle(string input, Handler<string> ifNext, Handler<int> next, CancellationToken ct = default)
    {
...
public class TestIfStepPipeline : IPipeline<string>
{
    ...
    public TestIfStepPipeline(IEnumerable<IMutator<Space>> mutators, Func<int, CancellationToken, Task> handler)
    {
        _handler = handler;
        var pipeline = CreateAndCompilePipeline();
        _compiledPipeline = pipeline.Compile(cfg =>
...
public class TestCheckIntOrFloatIfElseStep : IIfElseStep<string, string, int, int>
{
    ...
    public async Task Handle(string input, Handler<string> ifNext, Handler<int> elseNext, CancellationToken ct = default)
    {
        // Try to parse as int first
        if (int.TryParse(input, out var intValue))
        {
...
public class TestIfElseStepPipeline : IPipeline<string>
{
    ...
    public TestIfElseStepPipeline(IEnumerable<IMutator<Space>> mutators, Func<int, CancellationToken, Task> handler)
    {
        _handler = handler;
        var pipeline = CreateAndCompilePipeline();
        _compiledPipeline = pipeline.Compile(cfg =>
...
public class TestNumberRangeSwitchStep : ISwitchStep<string, int, int, int>
{
    ...
    public async Task Handle(string input, IReadOnlyDictionary<string, Handler<int>> cases, Handler<int> defaultNext, CancellationToken ct = default)
    {
...
public class TestSwitchStepPipeline : IPipeline<string>
{
    ...
    public TestSwitchStepPipeline(IEnumerable<IMutator<Space>> mutators, Func<int, CancellationToken, Task> handler)
    {
        _handler = handler;
        var pipeline = CreateAndCompilePipeline();
        _compiledPipeline = pipeline.Compile(cfg =>
    ...
    private Pipeline<string> CreateAndCompilePipeline()
    {
        ...
        var pipeline = Pipelines.CreateAsyncCancellablePipeline<string>("TestSwitchPipeline")
            .StartWithLinear(trimStep)
...
{
    private readonly Func<int, CancellationToken, Task> _intHandler;
    private readonly Func<string, CancellationToken, Task> _stringHandler;

    public TestDigitContentForkStep(Func<int, CancellationToken, Task> intHandler, Func<string, CancellationToken, Task> stringHandler)
    {
    ...
    public async Task Handle(string input, Handler<string> branchANext, Handler<string> branchBNext, CancellationToken ct = default)
    {
...
public class TestForkStepIntHandlerStep : IHandlerStep<int>
{
    ...
    public TestForkStepIntHandlerStep(Func<int, CancellationToken, Task> handler)
    {
        _handler = handler;
    }
...
public class TestForkStepStringHandlerStep : IHandlerStep<string>
{
    ...
    public TestForkStepStringHandlerStep(Func<string, CancellationToken, Task> handler)
    {
        _handler = handler;
    }
    ...
    public async Task Handle(string input, CancellationToken ct = default)
    {
        await _handler(input, ct);
    }
}
...
public class TestForkStepPipeline : IPipeline<string>
{
    ...
    public TestForkStepPipeline(IEnumerable<IMutator<Space>> mutators, Func<int, CancellationToken, Task> intHandler, Func<string, CancellationToken, Task> stringHandler)
    {
        _intHandler = intHandler;
        _stringHandler = stringHandler;
        var pipeline = CreateAndCompilePipeline();
...
public class TestMultiForkStepPipeline : IPipeline<string>
{
    ...
    private readonly Func<int, CancellationToken, Task> _intHandler;
    private readonly Func<string, CancellationToken, Task> _stringHandler;
    private readonly Func<char[], CancellationToken, Task> _charArrayHandler;

    public string Name => "TestMultiForkPipeline";

    public TestMultiForkStepPipeline(IEnumerable<IMutator<Space>> mutators, Func<int, CancellationToken, Task> intHandler, Func<string, CancellationToken, Task> stringHandler, Func<char[], CancellationToken, Task> charArrayHandler)
    {
        _intHandler = intHandler;
        _stringHandler = stringHandler;
...
public class TestMultiForkStep : IMultiForkStep<string, string, char[]>
{
    ...
    public async Task Handle(string input, IReadOnlyDictionary<string, Handler<string>> branches, Handler<char[]> defaultNext, CancellationToken ct = default)
    {
        var containsOnlyDigits = !string.IsNullOrEmpty(input) && input.All(char.IsDigit);
...
public class TestMultiForkIntHandlerStep : IHandlerStep<int>
{
    ...
    public TestMultiForkIntHandlerStep(Func<int, CancellationToken, Task> handler)
    {
        _handler = handler;
    }
...
public class TestMultiForkStringHandlerStep : IHandlerStep<string>
{
    ...
    public TestMultiForkStringHandlerStep(Func<string, CancellationToken, Task> handler)
    {
        _handler = handler;
    }
...
public class TestMultiForkConvertToCharArrayStep : ILinearStep<string, char[]>
{
    ...
    public async Task Handle(string input, Handler<char[]> next, CancellationToken ct = default)
    {
        var charArray = input.ToCharArray();
...
public class TestMultiForkRemoveDuplicatesStep : ILinearStep<char[], char[]>
{
    ...
    public async Task Handle(char[] input, Handler<char[]> next, CancellationToken ct = default)
    {
...
{
    private readonly Func<char[], CancellationToken, Task> _handler;

    public TestMultiForkCharArrayHandlerStep(Func<char[], CancellationToken, Task> handler)
    {
        _handler = handler;
    }
    ...
    public async Task Handle(char[] input, CancellationToken ct = default)
    {
        await _handler(input, ct);
    }
}
...
public class TestMultiForkCountDigitsAndLettersStep : ILinearStep<char[], (int DigitCount, int LetterCount)>
{
    ...
    public async Task Handle(char[] input, Handler<(int DigitCount, int LetterCount)> next, CancellationToken ct = default)
    {
...
public class TestMultiForkCalculateRatioStep : ILinearStep<(int DigitCount, int LetterCount), int>
{
    ...
    public async Task Handle((int DigitCount, int LetterCount) input, Handler<int> next, CancellationToken ct = default)
    {
...
public class TestMultiForkDefaultIntHandlerStep : IHandlerStep<int>
{
    ...
    public TestMultiForkDefaultIntHandlerStep(Func<int, CancellationToken, Task> handler)
    {
        _handler = handler;
    }
