...
public class WithMutationClassPipelineTests
{
    ...
    [InlineData(0, 1)]
    [InlineData(2, 3)]
    public void BuildAndRunPipeline_WhenOneHandlerStep_ShouldRun(int value, int expectedValue)
    {
        // Arrange
        var handler = Substitute.For<Func<int, ValueTask>>();
        handler.Invoke(Arg.Is(expectedValue)).Returns(ValueTask.CompletedTask);

        var mutator = new TestHandlerMutator();
        var testPipeline = new TestPipeline([mutator], handler);

        // Act
        testPipeline.Run(value);

        // Assert
        handler.Received().Invoke(Arg.Is(expectedValue));
    }
    ...
    public void BuildAndRunPipeline_WhenLinearStepThenHandlerStep_ShouldRun(int inputValue, int expectedHandlerInput)
    {
        // Arrange
        var handler = Substitute.For<Func<int, ValueTask>>();
    ...
    public void BuildAndRunPipeline_WhenTwoLinearStepsThenHandlerStep_ShouldRun(int inputValue, int expectedHandlerInput)
    {
        // Arrange
        var handler = Substitute.For<Func<int, ValueTask>>();
        handler.Invoke(Arg.Is(expectedHandlerInput)).Returns(ValueTask.CompletedTask);
    ...
    public void BuildAndRunPipeline_WhenStringParseAndAddConstant_ShouldCallHandlerOnlyOnSuccessfulParse(string inputValue, int expectedValue)
    {
        // Arrange
        var handler = Substitute.For<Func<int, ValueTask>>();
    ...
    public void BuildAndRunPipeline_WhenIfStepHandlesIntAndFloat_ShouldProcessCorrectly(string inputValue, int expectedResult)
    {
        // Arrange
        var handler = Substitute.For<Func<int, ValueTask>>();
    ...
    public void BuildAndRunPipeline_WhenIfElseStepHandlesIntFloatOrDefault_ShouldProcessCorrectly(string inputValue, int expectedResult)
    {
        // Arrange
        var handler = Substitute.For<Func<int, ValueTask>>();
        handler.Invoke(Arg.Is(expectedResult)).Returns(ValueTask.CompletedTask);
    ...
    public void Mutate(Space space)
    {
        ...
        var mutator = new StepMutator<Pipe<string, int>>("ParseToIntMutator", 1, (pipe) =>
        {
            return async (input, next) =>
            {
                ...
                if (int.TryParse(input, out var number))
                {
                    next(number + 5); // Add 5 to the parsed value
                }
                else
                {
                    next(0 + 5); // Even if parsing fails, add 5 to the default 0
                }
            };
        });
        step.Mutators.AddMutator(mutator, AddingMode.ExactPlace);
