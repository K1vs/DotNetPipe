...
public class TestReturningTwoStepPipeline
{
    ...
    {
        _space = Pipelines.CreateReturningAsyncSpace();
        _handler = handler;
    }

    public async Task<int> Run(int input, int constantToAdd)
    {
...
public class TestReturningThreeStepPipeline
{
    ...
    public async Task<int> Run(int input, int constantToAdd, int multiplier)
    {
...
{
    private readonly Func<string, int, Task<int>> _pipeline;
    ...
    public async Task<int> Run(string input, int constantToAdd)
    {
...
{
    private readonly Func<string, int, int, Task<int>> _pipeline;
    ...
    public async Task<int> Run(string input, int constantToAdd, int multiplier)
    {
...
{
    private readonly Func<string, Task<int>> _pipeline;
...
{
    private readonly Func<string, Task<(int?, string?)>> _pipeline;
    ...
    public async Task<(int?, string?)> Run(string input)
    {
        return await _pipeline(input);
    }
}
...
{
    private readonly Func<string, Task<(int?, string?, char[]?)>> _pipeline;
    ...
    public async Task<(int?, string?, char[]?)> Run(string input)
    {
        return await _pipeline(input);
    }
}
