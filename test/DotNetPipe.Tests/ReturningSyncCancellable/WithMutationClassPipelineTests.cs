...
public class WithMutationClassReturningPipelineTests
{
    ...
    [InlineData(0, 1)]
    [InlineData(2, 3)]
    public void BuildAndRunPipeline_WhenOneHandlerStep_ShouldReturn(int value, int expected)
    {
        var test = new TestReturningCancellableClassPipeline([
            new TestReturningCancellableClassHandlerMutator()
        ]);
        var result = test.Run(value, CancellationToken.None);
    ...
    [InlineData(0, 11)]
    [InlineData(2, 15)]
    public void BuildAndRunPipeline_WhenLinearStepThenHandlerStep_ShouldReturn(int input, int expected)
    {
        ...
        var result = test.Run(input, CancellationToken.None);
        Assert.Equal(expected, result);
    }
    ...
    [InlineData(0, 41)]
    [InlineData(-1, 39)]
    public void BuildAndRunPipeline_WhenTwoLinearStepsThenHandlerStep_ShouldReturn(int input, int expected)
    {
    ...
    [InlineData(" 2.3 ", 5)]
    [InlineData("5.5", 8)]
    public void BuildAndRunPipeline_WhenIfStepHandlesIntAndFloat_ShouldReturn(string inputValue, int expected)
    {
        ...
        var result = test.Run(inputValue, CancellationToken.None);
        Assert.Equal(expected, result);
    }
    ...
    [InlineData(" 2.3 ", 8)]
    [InlineData("5.5", 8)]
    public void BuildAndRunPipeline_WhenIfElseStepHandlesIntFloat_ShouldReturn(string inputValue, int expected)
    {
    ...
    [InlineData("hello", 8)]
    [InlineData("", 3)]
    public void BuildAndRunPipeline_WhenSwitchStepRoutesByNumberRange_ShouldReturn(string inputValue, int expected)
    {
    ...
    [InlineData("", 0, "******")]
    [InlineData("!@#", 0, "***!@#***")]
    public void BuildAndRunPipeline_WhenForkSplitsByDigitContent_ShouldReturn(string inputValue, int expectedInt, string? expectedString)
    {
    ...
    public void BuildAndRunPipeline_WhenMultiForkClassifiesStringContent_ShouldReturn(
        string inputValue,
...
public class TestReturningCancellableClassHandlerStep : IHandlerStep<int, int>
{
    ...
    public int Handle(int input, CancellationToken ct = default) => input;
}
...
public class TestReturningCancellableClassLinearStep : ILinearStep<int, int, int, int>
{
    ...
    public int Handle(int input, Handler<int, int> next, CancellationToken ct = default)
    {
        var result = input + ConstantToAdd;
        return next(result, ct);
    }
}
...
public class TestReturningCancellableClassTrimStep : ILinearStep<string, int, string, int>
{
    ...
    public int Handle(string input, Handler<string, int> next, CancellationToken ct = default)
    {
        return next(input.Trim(), ct);
    }
}
...
public class TestReturningCancellableClassIfStep : IIfStep<string, int, string, int, int, int>
{
    ...
    public int Handle(string input, Handler<string, int> ifNext, Handler<int, int> next, CancellationToken ct = default)
    {
        if (int.TryParse(input, out var intValue))
        {
            return next(intValue, ct);
        }
        return ifNext(input, ct);
    }
    public OpenPipeline<string, int, int, int> BuildTruePipeline(Space space)
    {
        return space.CreatePipeline<string, int>("FloatProcessing")
            .StartWithLinear<double, int>("ParseFloat", (val, nxt, ct) =>
            {
                if (double.TryParse(val, out var f)) return nxt(f, ct);
                return 0;
            })
            .ThenLinear<int, int>("RoundToInt", (val, nxt, ct) =>
            {
                var rounded = (int)Math.Round(val);
                return nxt(rounded, ct);
            })
            .BuildOpenPipeline();
    }
}
...
{
    public string Name => "CheckIntOrFloat";
    public int Handle(string input, Handler<string, int> ifNext, Handler<int, int> elseNext, CancellationToken ct = default)
    {
        if (int.TryParse(input, out var intValue))
        {
            return elseNext(intValue, ct);
        }
        return ifNext(input, ct);
    }
    public OpenPipeline<string, int, int, int> BuildTruePipeline(Space space)
    ...
    public OpenPipeline<int, int, int, int> BuildFalsePipeline(Space space)
    {
        return space.CreatePipeline<int, int>("IntProcessing")
            .StartWithLinear<int, int>("Multiply", (val, nxt, ct) => nxt(val * 2, ct))
            .BuildOpenPipeline();
    }
}
...
public class TestReturningCancellableClassNumberRangeSwitchStep : ISwitchStep<string, int, int, int, int, int, int, int>
{
    ...
    public int Handle(string input, IReadOnlyDictionary<string, Handler<int, int>> cases, Handler<int, int> defaultNext, CancellationToken ct = default)
    {
...
public class TestReturningCancellableClassPipeline
{
    ...
    public int Run(int input, CancellationToken ct) => _compiled(input, ct);
}
...
public class TestReturningCancellableClassIfStepPipeline
{
    ...
    public int Run(string input, CancellationToken ct) => _compiled(input, ct);
}
...
public class TestReturningCancellableClassLinearMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mutator = new StepMutator<Pipe<int, int, int, int>>("AddConstantMutator", 1, pipe => (input, next, ct) => pipe(input * 2, next, ct));
        step.Mutators.AddMutator(mutator, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassFirstLinearMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mutator = new StepMutator<Pipe<int, int, int, int>>("AddConstantMutator", 1, pipe => (input, next, ct) => pipe(input + 5, next, ct));
        step.Mutators.AddMutator(mutator, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassSecondLinearMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mutator = new StepMutator<Pipe<int, int, int, int>>("MultiplyByCoefficient", 1, pipe => (input, next, ct) => pipe(input + 5, next, ct));
        step.Mutators.AddMutator(mutator, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassIfTrimMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Pipe<string, int, string, int>>("TrimStringMutator", 1, pipe => (input, next, ct) => pipe(input, next, ct));
...
public class TestReturningCancellableClassIfSelectorMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<IfSelector<string, int, string, int, int, int>>("CheckIntOrFloatMutator", 1, sel => (input, ifNext, next, ct) => ifNext(input, ct));
        step.Mutators.AddMutator(mut, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassIfRoundToIntMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Pipe<double, int, int, int>>("RoundToIntMutator", 1, pipe => (input, next, ct) =>
        {
            input += 1;
...
public class TestReturningCancellableClassIfAddConstantMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Pipe<int, int, int, int>>("AddConstantMutator", 1, pipe => (input, next, ct) => pipe(input, next, ct));
        step.Mutators.AddMutator(mut, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassIfHandlerMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Handler<int, int>>("TestHandlerMutator", 1, handler => (input, ct) => handler(input, ct));
        step.Mutators.AddMutator(mut, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassIfElseSelectorMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<IfElseSelector<string, int, string, int, int, int>>("CheckIntOrFloatMutator", 1, sel => (input, trueNext, falseNext, ct) =>
        {
...
public class TestReturningCancellableClassIfElseParseFloatMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Pipe<string, int, double, int>>("ParseFloatMutator", 1, pipe => (input, next, ct) => pipe(input, next, ct));
        step.Mutators.AddMutator(mut, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassIfElseMultiplyMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Pipe<int, int, int, int>>("IntProcessingMutator", 1, pipe => (input, next, ct) => pipe(input + 2, next, ct));
        step.Mutators.AddMutator(mut, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassIfElseHandlerMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Handler<int, int>>("TestHandlerMutator", 1, handler => (input, ct) => handler(input + 1, ct));
        step.Mutators.AddMutator(mut, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassSwitchSelectorMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<SwitchSelector<string, int, int, int, int, int>>("NumberRangeSwitchMutator", 1, selector => (input, cases, defaultNext, ct) =>
        {
            if (int.TryParse(input, out var number))
            {
...
public class TestReturningCancellableClassSwitchMultiplyByThreeMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Pipe<int, int, int, int>>("MultiplyByThreeMutator", 1, pipe => (input, next, ct) => pipe(input, next, ct));
        step.Mutators.AddMutator(mut, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassSwitchAddTwoMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Pipe<int, int, int, int>>("AddTwoMutator", 1, pipe => (input, next, ct) => pipe(input, next, ct));
        step.Mutators.AddMutator(mut, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassSwitchMultiplyByTwoMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Pipe<int, int, int, int>>("MultiplyByTwoMutator", 1, pipe => (input, next, ct) => pipe(input, next, ct));
        step.Mutators.AddMutator(mut, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassSwitchKeepZeroMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Pipe<int, int, int, int>>("KeepZeroMutator", 1, pipe => (input, next, ct) => pipe(input, next, ct));
        step.Mutators.AddMutator(mut, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassSwitchStringLengthMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Pipe<int, int, int, int>>("StringLengthMutator", 1, pipe => (input, next, ct) => pipe(input, next, ct));
        step.Mutators.AddMutator(mut, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassSwitchHandlerMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Handler<int, int>>("TestHandlerMutator", 1, handler => (input, ct) => handler(input + 3, ct));
        step.Mutators.AddMutator(mut, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassDigitContentForkStep : IForkStep<string, (int?, string?), string, (int?, string?), string, (int?, string?)>
{
    ...
    public (int?, string?) Handle(string input, Handler<string, (int?, string?)> digitBranch, Handler<string, (int?, string?)> nonDigitBranch, CancellationToken ct = default)
    {
        var onlyDigits = !string.IsNullOrEmpty(input) && input.All(char.IsDigit);
        return onlyDigits ? digitBranch(input, ct) : nonDigitBranch(input, ct);
    }
    public Pipeline<string, (int?, string?)> BuildBranchAPipeline(Space space)
    {
        return space.CreatePipeline<string, (int?, string?)>("DigitProcessing")
            ...
            {
                if (int.TryParse(input, out var number)) return next(number, ct);
                return next(0, ct);
            })
...
public class TestReturningCancellableClassForkStepPipeline
{
    ...
    public (int?, string?) Run(string input, CancellationToken ct) => _compiled(input, ct);
}
...
public class TestReturningCancellableClassForkTrimMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Pipe<string, (int?, string?), string, (int?, string?)>>("TrimStringMutator", 1, pipe => (input, next, ct) => pipe(input, next, ct));
        step.Mutators.AddMutator(mut, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassForkSelectorMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<ForkSelector<string, (int?, string?), string, (int?, string?), string, (int?, string?)>>("DigitContentForkMutator", 1, selector => (input, digitBranch, nonDigitBranch, ct) =>
        {
            if (input.Length > 3) return digitBranch(input, ct);
...
public class TestReturningCancellableClassForkRemoveNonDigitsMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Pipe<string, (int?, string?), string, (int?, string?)>>("RemoveNonDigitsMutator", 1, pipe => (input, next, ct) => pipe(input, next, ct));
        step.Mutators.AddMutator(mut, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassForkParseToIntMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Pipe<string, (int?, string?), int, (int?, string?)>>("ParseToIntMutator", 1, pipe => (input, next, ct) =>
        {
            if (int.TryParse(input, out var number)) return next(number + 5, ct);
            return next(0 + 5, ct);
        });
        step.Mutators.AddMutator(mut, AddingMode.ExactPlace);
    }
...
public class TestReturningCancellableClassForkIntHandlerMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Handler<int, (int?, string?)>>("IntHandlerMutator", 1, handler => (input, ct) => handler(input, ct));
        step.Mutators.AddMutator(mut, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassForkRemoveDigitsMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Pipe<string, (int?, string?), string, (int?, string?)>>("RemoveDigitsMutator", 1, pipe => (input, next, ct) => pipe(input, next, ct));
        step.Mutators.AddMutator(mut, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassForkAddSpacesMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Pipe<string, (int?, string?), string, (int?, string?)>>("AddSpacesMutator", 1, pipe => (input, next, ct) => next($"***{input}***", ct));
        step.Mutators.AddMutator(mut, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassForkStringHandlerMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Handler<string, (int?, string?)>>("StringHandlerMutator", 1, handler => (input, ct) => handler(input, ct));
        step.Mutators.AddMutator(mut, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassMultiForkStep : IMultiForkStep<string, (int?, string?, char[]?), string, (int?, string?, char[]?), char[], (int?, string?, char[]?)>
{
    ...
    public (int?, string?, char[]?) Handle(string input, IReadOnlyDictionary<string, Handler<string, (int?, string?, char[]?)>> branches, Handler<char[], (int?, string?, char[]?)> defaultNext, CancellationToken ct = default)
    {
        var onlyDigits = !string.IsNullOrEmpty(input) && input.All(char.IsDigit);
        ...
        if (onlyDigits) return branches["DigitBranch"](input, ct);
        if (onlyLetters) return branches["LetterBranch"](input, ct);
        if (onlySpecial) return branches["SpecialCharBranch"](input, ct);
...
public class TestReturningCancellableClassMultiForkStepPipeline
{
    ...
    public (int?, string?, char[]?) Run(string input, CancellationToken ct) => _compiled(input, ct);
}
...
public class TestReturningCancellableClassMultiForkTrimMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Pipe<string, (int?, string?, char[]?), string, (int?, string?, char[]?)>>("TrimStringMutator", 1, pipe => (input, next, ct) => pipe(input, next, ct));
        step.Mutators.AddMutator(mut, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassMultiForkSelectorMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<MultiForkSelector<string, (int?, string?, char[]?), string, (int?, string?, char[]?), char[], (int?, string?, char[]?)>>("ClassifyStringContentMutator", 1, selector => (input, branches, defaultNext, ct) =>
        {
            var onlyDigits = !string.IsNullOrEmpty(input) && input.All(char.IsDigit);
            if (onlyDigits) return branches["DigitBranch"](input, ct);
...
public class TestReturningCancellableClassMultiForkParseToIntMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Pipe<string, (int?, string?, char[]?), int?, (int?, string?, char[]?)>>("ParseStringToIntMutator", 1, pipe => (input, next, ct) => pipe(input, next, ct));
        step.Mutators.AddMutator(mut, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassMultiForkAddConstantMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Pipe<int?, (int?, string?, char[]?), int?, (int?, string?, char[]?)>>("AddConstantMutator", 1, pipe => (input, next, ct) => next((input ?? 0) + 10 + 5, ct));
        step.Mutators.AddMutator(mut, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassMultiForkIntHandlerMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Handler<int?, (int?, string?, char[]?)>>("IntHandlerMutatorDigit", 1, handler => (input, ct) => handler(input, ct));
        step.Mutators.AddMutator(mut, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassMultiForkAddSpacesMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Pipe<string, (int?, string?, char[]?), string, (int?, string?, char[]?)>>("AddSpacesMutator", 1, pipe => (input, next, ct) => next($"***{input}***", ct));
        step.Mutators.AddMutator(mut, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassMultiForkStringHandlerMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Handler<string, (int?, string?, char[]?)>>("StringHandlerMutator", 1, handler => (input, ct) => handler(input, ct));
        step.Mutators.AddMutator(mut, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassMultiForkRemoveWhitespaceMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Pipe<string, (int?, string?, char[]?), string, (int?, string?, char[]?)>>("RemoveWhitespaceMutator", 1, pipe => (input, next, ct) => pipe(input, next, ct));
        step.Mutators.AddMutator(mut, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassMultiForkConvertToCharArrayMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Pipe<string, (int?, string?, char[]?), char[], (int?, string?, char[]?)>>("ConvertToCharArrayMutator", 1, pipe => (input, next, ct) => next((input + "_").ToCharArray(), ct));
        step.Mutators.AddMutator(mut, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassMultiForkRemoveDuplicatesMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Pipe<char[], (int?, string?, char[]?), char[], (int?, string?, char[]?)>>("RemoveDuplicatesMutator", 1, pipe => (input, next, ct) => pipe(input, next, ct));
        step.Mutators.AddMutator(mut, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassMultiForkCountDigitsAndLettersMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Pipe<char[], (int?, string?, char[]?), (int DigitCount, int LetterCount), (int?, string?, char[]?)>>("CountDigitsAndLettersMutator", 1, pipe => (input, next, ct) => pipe(input, next, ct));
        step.Mutators.AddMutator(mut, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassMultiForkCalculateRatioMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Pipe<(int DigitCount, int LetterCount), (int?, string?, char[]?), int, (int?, string?, char[]?)>>("CalculateRatioMutator", 1, pipe => (input, next, ct) =>
        {
            var ratio = input.LetterCount > 0 ? input.DigitCount / input.LetterCount : input.DigitCount;
            return next(ratio + 2, ct);
        });
        step.Mutators.AddMutator(mut, AddingMode.ExactPlace);
    }
}
...
public class TestReturningCancellableClassMultiForkDefaultIntHandlerMutator : IMutator<Space>
{
    public void Mutate(Space space)
    {
        ...
        var mut = new StepMutator<Handler<int, (int?, string?, char[]?)>>("IntHandlerMutatorDefault", 1, handler => (input, ct) => handler(input, ct));
        step.Mutators.AddMutator(mut, AddingMode.ExactPlace);
    }
}
